...
export class CardataService {
  ...
  private CarSubject: Subject<ECarsData[]> = new BehaviorSubject<ECarsData[]>([]);

  CarOberserver: Observable<ECarsData[]> = this.CarSubject.asObservable();

  timer: any;

  constructor() { 

    let temp: Array<ECarsData> = new Array<ECarsData>();
    ...
    //this.CarSubject.next(this.eCarsTop25_2022);

    this.timer = setInterval(() => {

      if(temp.length != this.eCarsTop25_2022.length){
        temp.push(this.eCarsTop25_2022[temp.length]);
        this.CarSubject.next(temp);
      }else{
